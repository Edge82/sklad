...
3. Отсканируйте штрихкоды товаров — они добавляются в таблицу состава заказа.
4. При необходимости измените **количество** или удалите позиции.
5. Нажмите **«Сохранить и отправить в 1С»**.
   - ✅ Если 1С доступна — заказ сразу создаётся со статусом «Завершён», модальное окно закрывается.
   - ⚠️ Если 1С недоступна — заказ сохраняется локально с префиксом `LOCAL-`, кнопка отправки блокируется.
